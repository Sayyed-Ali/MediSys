const express = require('express');
const router = express.Router();
const axios = require('axios');
const FormData = require('form-data');
const multer = require('multer');
const { Types } = require('mongoose');

const Inventory = require('../models/Inventory');
const Medicine = require('../models/Medicine');
const AuditLog = require('../models/AuditLog');
const InvoiceReview = require('../models/InvoiceReview');
const auth = require('../middleware/auth');
const checkRole = require('../middleware/checkRole');
const { matchMedicineByName } = require('../utils/matchMedicine');

const upload = multer({ storage: multer.memoryStorage() });

const INVOICE_SERVICE_URL = process.env.INVOICE_SERVICE_URL || 'http://127.0.0.1:5001/api/invoice/parse';
const AUTO_MATCH_THRESHOLD = 0.80;
const REVIEW_THRESHOLD = 0.60;

/** Helper: parse expiry text to Date or null */
function parseExpiryToDate(text) {
  if (!text) return null;
  const s = String(text).trim();

  // MM/YYYY or MM-YYYY -> last day of month
  let m = s.match(/^(\d{1,2})[\/\-](\d{4})$/);
  if (m) {
    const mm = parseInt(m[1], 10);
    const yyyy = parseInt(m[2], 10);
    if (mm >= 1 && mm <= 12) {
      const date = new Date(yyyy, mm, 0);
      if (!isNaN(date.getTime())) return date;
    }
  }

  // YYYY-MM-DD or YYYY/MM/DD
  m = s.match(/^(\d{4})[\/\-](\d{1,2})[\/\-](\d{1,2})$/);
  if (m) {
    const d = new Date(s);
    if (!isNaN(d.getTime())) return d;
  }

  // MMM YYYY or other parseable strings
  try {
    const parsed = new Date(s);
    if (!isNaN(parsed.getTime())) return parsed;
  } catch (e) { }

  // fallback: find 4-digit year
  const y = s.match(/(20\d{2}|19\d{2})/);
  if (y) {
    const year = parseInt(y[1], 10);
    const date = new Date(year, 11, 31);
    if (!isNaN(date.getTime())) return date;
  }

  return null;
}

/** Helper: escape regex */
function escapeRegExp(string) {
  return string.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

/**
 * POST /api/invoice/upload
 * - Accepts multipart 'invoice' file and optional supplierId
 * - Forwards file to OCR service, receives rows
 * - Tries to auto-add matched rows, saves needsReview rows into InvoiceReview
 */
router.post('/upload', auth, checkRole(['Admin']), upload.single('invoice'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ msg: 'No invoice PDF uploaded (field name: invoice).' });

    // Send to OCR microservice
    const form = new FormData();
    form.append('file', req.file.buffer, {
      filename: req.file.originalname || 'invoice.pdf',
      contentType: req.file.mimetype || 'application/pdf',
    });

    console.log('[invoiceRoutes] Posting to:', INVOICE_SERVICE_URL);

    const invoiceResp = await axios.post(INVOICE_SERVICE_URL, form, {
      headers: { ...form.getHeaders() },
      validateStatus: () => true,
      timeout: 600000,
    });

    console.log('[invoiceRoutes] Invoice service status:', invoiceResp.status);
    if (invoiceResp.status !== 200) {
      console.error('[invoiceRoutes] Invoice service error body:', invoiceResp.data);
      return res.status(500).json({ error: `Invoice service ${invoiceResp.status}`, details: invoiceResp.data });
    }

    if (!invoiceResp.data || !Array.isArray(invoiceResp.data.rows)) {
      console.error('[invoiceRoutes] Unexpected response shape:', invoiceResp.data);
      return res.status(500).json({ msg: 'Invoice service returned unexpected response', raw: invoiceResp.data });
    }

    const rows = invoiceResp.data.rows;
    const autoAdded = [];
    const needsReview = [];

    const supplierId = (req.body && req.body.supplierId) ? new Types.ObjectId(req.body.supplierId) : undefined;

    for (const r of rows) {
      const desc = (r.description || '').trim();
      const qty = Number(r.quantity || 0);
      const batch = (r.batch || '').trim() || 'UNKNOWN';
      const expiryRaw = r.expiry || null;
      const price = r.price ? Number(r.price) : null;

      if (!desc || !qty || qty <= 0) {
        needsReview.push({ row: r, reason: 'missing description or non-positive quantity' });
        continue;
      }

      // If medicine master list is empty -> create new Medicine doc and treat as match
      let matchedMedicine = null;
      let rating = 0;

      const medCount = await Medicine.countDocuments().catch(e => {
        console.warn('[invoiceRoutes] Medicine countDocuments error:', e && e.message ? e.message : e);
        return 0;
      });

      if (!medCount) {
        // create or find by exact name (case-insensitive)
        const nameExact = desc;
        matchedMedicine = await Medicine.findOne({ name: { $regex: new RegExp(`^${escapeRegExp(nameExact)}$`, 'i') } });
        if (!matchedMedicine) {
          const newMed = new Medicine({
            name: nameExact,
            brand: (r.brand || 'Unknown') || 'Unknown',
            form: r.form || 'Unknown',
            strength: r.strength || ''
          });
          await newMed.save();
          matchedMedicine = newMed;
          console.log('[invoiceRoutes] Created medicine from invoice (master list empty):', newMed.name);
        } else {
          console.log('[invoiceRoutes] Found existing medicine by exact name (master list empty fallback):', matchedMedicine.name);
        }
        rating = 1;
      } else {
        // Normal fuzzy match path when medicines exist
        const match = await matchMedicineByName(desc);
        rating = match?.rating ?? 0;
        matchedMedicine = match?.medicine ?? null;
      }

      // If good match -> update-or-create inventory
      if (rating >= AUTO_MATCH_THRESHOLD && matchedMedicine) {
        const parsedExpiry = parseExpiryToDate(expiryRaw);

        let updatedInventory = null;

        // 1) exact expiry match
        if (parsedExpiry) {
          try {
            updatedInventory = await Inventory.findOneAndUpdate(
              {
                medicine: matchedMedicine._id,
                batchNumber: batch,
                expiryDate: parsedExpiry,
              },
              { $inc: { quantity: qty } },
              { new: true }
            );
          } catch (err) {
            console.warn('[invoiceRoutes] Exact expiry findOneAndUpdate failed (will try range match):', err && err.message ? err.message : err);
          }

          // 2) fallback: same month/year range
          if (!updatedInventory) {
            const start = new Date(parsedExpiry.getFullYear(), parsedExpiry.getMonth(), 1);
            const end = new Date(parsedExpiry.getFullYear(), parsedExpiry.getMonth() + 1, 0, 23, 59, 59, 999);
            updatedInventory = await Inventory.findOneAndUpdate(
              {
                medicine: matchedMedicine._id,
                batchNumber: batch,
                expiryDate: { $gte: start, $lte: end },
              },
              { $inc: { quantity: qty } },
              { new: true }
            );
          }
        }

        // 3) try medicine + batch (no expiry)
        if (!updatedInventory) {
          updatedInventory = await Inventory.findOneAndUpdate(
            {
              medicine: matchedMedicine._id,
              batchNumber: batch,
            },
            { $inc: { quantity: qty } },
            { new: true }
          );
        }

        // 4) create new if nothing matched
        if (updatedInventory) {
          autoAdded.push({ row: r, inventoryId: updatedInventory._id, medicine: matchedMedicine.name, rating });
        } else {
          const invObj = {
            medicine: matchedMedicine._id,
            batchNumber: batch,
            quantity: qty,
          };
          if (parsedExpiry) invObj.expiryDate = parsedExpiry;
          if (supplierId) invObj.supplier = supplierId;

          const newInv = new Inventory(invObj);
          await newInv.save();
          autoAdded.push({ row: r, inventoryId: newInv._id, medicine: matchedMedicine.name, rating });
        }
      } else if (rating >= REVIEW_THRESHOLD && matchedMedicine) {
        // candidate found but not confident enough
        needsReview.push({ row: r, candidateMatches: [{ medicine: matchedMedicine, rating }], rating });
      } else {
        // no suitable candidate
        const matchCandidate = await matchMedicineByName(desc); // attempt for candidate listing
        needsReview.push({ row: r, candidateMatches: matchCandidate ? [{ medicine: matchCandidate.medicine, rating: matchCandidate.rating }] : [], rating: matchCandidate?.rating ?? 0 });
      }
    } // end loop rows

    // Persist review items so admin can review later
    for (const n of needsReview) {
      try {
        await InvoiceReview.create({
          description: n.row.description || n.row.desc || '',
          batch: n.row.batch || '',
          expiry: n.row.expiry || '',
          quantity: Number(n.row.quantity || 0),
          price: n.row.price ? Number(n.row.price) : null,
          raw: n.row.raw || [],
          candidateMatches: (n.candidateMatches || []).map(c => ({
            medicine: c.medicine ? c.medicine._id : undefined,
            rating: c.rating || 0
          })).filter(x => x.medicine)
        });
      } catch (e) {
        console.warn('[invoiceRoutes] Failed to create InvoiceReview:', e && e.message ? e.message : e);
      }
    }

    // Audit log
    const audit = new AuditLog({
      action: 'invoice_import',
      user: req.user?.id || null,
      summary: {
        fileName: req.file.originalname,
        autoAddedCount: autoAdded.length,
        needsReviewCount: needsReview.length,
      },
      rawRowsCount: rows.length,
      rawResponse: invoiceResp.data,
    });
    await audit.save();

    return res.status(200).json({ msg: 'Invoice processed', autoAdded, needsReviewCount: needsReview.length, auditId: audit._id });
  } catch (err) {
    console.error('Invoice upload error:', (err && err.message) ? err.message : err);
    try { console.error('Full error object:', JSON.stringify(err, Object.getOwnPropertyNames(err))); } catch (e) { }
    if (err.response) {
      console.error('Invoice service response (status):', err.response.status);
      console.error('Invoice service response (data):', err.response.data);
      console.error('Invoice service response (headers):', err.response.headers);
    }
    return res.status(500).json({ error: err.message || 'Server error while processing invoice.' });
  }
});

/* -------------------
   Review endpoints
   ------------------- */

// GET /api/invoice/review/pending
router.get('/review/pending', auth, checkRole(['Admin']), async (req, res) => {
  try {
    const items = await InvoiceReview.find({ status: 'pending' })
      .populate('candidateMatches.medicine')
      .sort({ createdAt: 1 })
      .lean();
    res.json(items);
  } catch (err) {
    console.error('[invoiceRoutes] GET review/pending error:', err && err.message ? err.message : err);
    res.status(500).json({ error: 'Server error fetching pending reviews' });
  }
});

// POST /api/invoice/review/approve/:id
router.post('/review/approve/:id', auth, checkRole(['Admin']), async (req, res) => {
  try {
    const review = await InvoiceReview.findById(req.params.id);
    if (!review) return res.status(404).json({ msg: 'Review item not found' });

    // req.body may contain medicineId to link to existing, or newMedicineName to create
    let medId = req.body.medicineId;
    if (!medId && req.body.newMedicineName) {
      const nm = req.body.newMedicineName.trim();
      const existing = await Medicine.findOne({ name: { $regex: new RegExp(`^${escapeRegExp(nm)}$`, 'i') } });
      if (existing) medId = existing._id;
      else {
        const newMed = new Medicine({ name: nm, brand: req.body.brand || 'Unknown', form: req.body.form || 'Unknown' });
        await newMed.save();
        medId = newMed._id;
      }
    }

    // If still no medId, try to use candidateMatches[0]
    if (!medId && review.candidateMatches && review.candidateMatches.length) {
      medId = review.candidateMatches[0].medicine;
    }

    if (!medId) return res.status(400).json({ msg: 'No medicine provided to approve.' });

    const expiry = parseExpiryToDate(review.expiry);
    let updatedInventory = null;
    const qty = Number(review.quantity || 0);

    // try to increment existing inventory similar to upload logic
    if (expiry) {
      try {
        updatedInventory = await Inventory.findOneAndUpdate(
          { medicine: medId, batchNumber: review.batch, expiryDate: expiry },
          { $inc: { quantity: qty } },
          { new: true }
        );
      } catch (e) { /* ignore */ }

      if (!updatedInventory) {
        const start = new Date(expiry.getFullYear(), expiry.getMonth(), 1);
        const end = new Date(expiry.getFullYear(), expiry.getMonth() + 1, 0, 23, 59, 59, 999);
        updatedInventory = await Inventory.findOneAndUpdate(
          { medicine: medId, batchNumber: review.batch, expiryDate: { $gte: start, $lte: end } },
          { $inc: { quantity: qty } },
          { new: true }
        );
      }
    }

    if (!updatedInventory) {
      updatedInventory = await Inventory.findOneAndUpdate(
        { medicine: medId, batchNumber: review.batch },
        { $inc: { quantity: qty } },
        { new: true }
      );
    }

    if (!updatedInventory) {
      const invObj = { medicine: medId, batchNumber: review.batch, quantity: qty };
      if (expiry) invObj.expiryDate = expiry;
      const created = new Inventory(invObj);
      await created.save();
      updatedInventory = created;
    }

    review.status = 'approved';
    await review.save();

    res.json({ msg: 'Approved and inventory updated', inventoryId: updatedInventory._id });
  } catch (err) {
    console.error('[invoiceRoutes] Approve error:', err && err.message ? err.message : err);
    res.status(500).json({ error: 'Server error approving review item' });
  }
});

// POST /api/invoice/review/reject/:id
router.post('/review/reject/:id', auth, checkRole(['Admin']), async (req, res) => {
  try {
    const review = await InvoiceReview.findById(req.params.id);
    if (!review) return res.status(404).json({ msg: 'Review item not found' });
    review.status = 'rejected';
    await review.save();
    res.json({ msg: 'Rejected' });
  } catch (err) {
    console.error('[invoiceRoutes] Reject error:', err && err.message ? err.message : err);
    res.status(500).json({ error: 'Server error rejecting review item' });
  }
});

module.exports = router;